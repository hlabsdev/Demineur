...
namespace TepeGolo{
	...
	void Grille::Update(float dt){
        ...
        if(gameState == STATE_WON){
            if(this->_temps.getElapsedTime().asSeconds() > TEMPS_AVANT_GAME_OVER){
                this->_data->machine.AjoutEtat(EtatRef(new GameWinState(_data)), true);
            }
        }
	...
	void Grille::ClickGauche(){
        this->GetPosition();
        if (gameState == STATE_PLAYING && _cases[this->_x][this->_y]._estClicable == true){
            if(this->firstClick != true){
	...
	void Grille::ClickDroit(){
        ...
        if (gameState == STATE_PLAYING && _cases[this->_x][this->_y]._estClicable == true ){
            if(!_cases[this->_x][this->_y]._estMarque && !_cases[this->_x][this->_y]._estDecouvers && nbMine > 0){
                    //marquer
	...
	void Grille::GenererMine(){
        ...
        while (mineRestantAPlacer > 0) {
            ...
            if(j <10 && mineRestantAPlacer == 0){
                mineRestantAPlacer+=1;
            }
        }
	}
	    ...
	    for(int x=0; x<9; x++){
            for(int y=0; y<9; y++){
                if(_cases[x][y]._estMinee){
                    ...
                    _cases[x][y]._estClicable = false;
                }
                //Montrer les fausses bombes
                if(_cases[x][y]._estMarque && !_cases[x][y]._estMinee){
                    _cases[x][y].FausseBombe();
                }
